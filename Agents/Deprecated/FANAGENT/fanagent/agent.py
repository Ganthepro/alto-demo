...
def fanagent(config_path, **kwargs):
    ...
    return Fanagent(hotel_name, building_name, room_no, device_name, mac_address, topic_head,
...
class Fanagent(Agent):
    ...
    def __init__(self, hotel_name, building_name, room_no, device_name, mac_address, topic_head,
                       mqtt_topics, mqtt_server, mqtt_port, mqtt_user, mqtt_password, message_agent, **kwargs):
        super(Fanagent, self).__init__(**kwargs)
    ...
    def _create_subscriptions(self, topic):
        ...
        self.vip.pubsub.subscribe(peer='pubsub',
                                  prefix=topic,
                                  callback=self.fancmd)

    def fancmd(self, peer, sender, bus, topic, headers, message):
        _log.debug("Print from subscribe lightcmd :{}, sender:{}, bus:{}, topic:{},headers:{} , message:{}"
                      .format(peer, sender, bus, topic, headers, message))
        _log.debug("{}-->{}".format(headers["request"], type(headers["request"])))

        if "controlfan" in headers["request"]:                 #controllight message = {"index_status":{1:'on'}}
            for index, val in message["index_status"].items():
        ...
        if "statusfan" in headers["request"]:                #message = {"index_status":{1:,}} not use value just key
            for index in message["index_status"].keys():
                dev = self.device_name + "_" + self.mac_address[6:].upper()
                mqtttopic = "cmnd/tasmota/{}/Power{}".format(dev, index)
