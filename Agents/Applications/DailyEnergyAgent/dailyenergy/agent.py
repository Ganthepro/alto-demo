...
class ListenerAgent(Agent):
    ...
    def prepare_dataframe(self, data: list):
        try:
            ...
            print(f"prepare_dataframe error: {e}")

    def today_energy_consumption(self, df) -> dict:
        try:
            kwh = []
            kwh_each_room = {}
            for device_id in df['device_id'].unique():
                ...
                data = {'today_energy_consumption': {
                    'value': e,
                    'updated_at': pendulum.now('Asia/Bangkok').int_timestamp
                }}
                if device_id.split(':')[-1] == '8':
                    kwh_each_room.update({'601': data})
                elif device_id.split(':')[-1] == '9':
                    kwh_each_room.update({'602': data})
                elif device_id.split(':')[-1] == '10':
                    kwh_each_room.update({'603': data})
                elif device_id.split(':')[-1] == '11':
                    kwh_each_room.update({'604': data})
                elif device_id.split(':')[-1] == '12':
                    kwh_each_room.update({'605': data})

            # print(kwh_each_room)
            data = {
                'value': round(sum(kwh), 2),
                'unit': 'kWh',
                'updated_at': pendulum.now('Asia/Bangkok').int_timestamp
            }

            return {'daily_energy_consumption': data}, kwh_each_room
        except Exception as e:
            print(f"today_energy_consumption error: {e}")

    def update_firebase(self, daily_data):
        db.child("building").child("pmcu").child("pages").child("dashboard").update(daily_data)

    def update_each_roomfirebase(self, kwh_each_room):
        for room_number, data in kwh_each_room.items():
            db.child("building").child("pmcu").child("pages").child("floor_usage").child("rooms").child(
                room_number).update(data)
    ...
    def onsetup(self, sender, **kwargs):
        ...
        # Step 3: Calculate today energy consumption
        # Step 4: Update Firebase

        try:
            # Step 1
            start_of_today = pendulum.today('Asia/Bangkok').int_timestamp
            results = self.get_dataset_from_cosmosdb(gatewayid='altopicuterracemonitor', from_when=start_of_today)

            # Step 2
            df_elec = self.prepare_dataframe(results)

            # Step 3
            today_data, kwh_each_room = self.today_energy_consumption(df_elec)

            # Step 4: Update Firebase
            self.update_firebase(today_data)
            self.update_each_roomfirebase(kwh_each_room)
    ...
    @Core.schedule(periodic(1800)) # update every 30 mins
