    ...
    def get_battery_data(self, battery_id):
        try:
            if self.db is not None:
                ...
                state_of_charge = battery.val().get('state_of_charge')
                timestamp = battery.val().get('timestamp')
    ...
    def get_evbike_data(self, evbike_id):
        try:
            if self.db is not None:
                ...
                speed = evbike.val().get('speed')
                timestamp = evbike.val().get('timestamp')
    ...
    def get_ltracker_data(self, ltracker_id):
        try:
            if self.db is not None:
                ...
                lat = ltracker.val().get('lat')
                lon = ltracker.val().get('lon')
                timestamp = ltracker.val().get('timestamp')
