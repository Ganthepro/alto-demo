...
def got_cov(elements=None):
    ...
    if addr == 258:
        ...
        print(f"""addr {addr} {elements["properties"]["presentValue"]}""")
bacnet = BAC0.lite(ip="192.168.2.103", port=0xBAC0)

# res = bacnet.read(cmd)
# bacnet.cov("192.168.0.10", ("multiStateOutput", 5), lifetime=10, callback = got_cov) # lifetime=None
# bacnet.cov("192.168.2.110", ("binaryOutput", 386), callback = got_cov) # StartStopCommand_000
bacnet.cov("192.168.2.110", ("binaryInput", 258), callback = got_cov) # StartStopStatus_000
